...
export interface CoverflowCarouselProps {
  ...
  fade?: number;
  /** Extra scale applied to the centred card, tapering off by the first neighbour. */
  activeScale?: number;
  /** Pixels the centred card lifts by, tapering off by the first neighbour. */
  activeLift?: number;
  /** Any CSS length. Everything else is derived from it, so the rake scales. */
  cardWidth?: string;
  /** Card height. Defaults to `cardWidth` (square cards). */
  cardHeight?: string;
  ...
  /** Custom content per card — replaces the default `<img>` rendering. */
  renderSlide?: (
    slide: CoverflowSlide,
    index: number,
    isActive: boolean,
  ) => React.ReactNode;
  /** Fires whenever the centred slide changes (settle or drag). */
...
>(function CoverflowCarousel(
  ...
  const dragRef = React.useRef<{
    ...
    v: number;
    t: number;
    /** Past the tap tolerance — only then do we capture the pointer and
        move cards, so a plain click on a card still reaches its onClick. */
    moved: boolean;
  } | null>(null);
  ...
  const paint = React.useCallback(() => {
    ...
    cardRefs.current.forEach((card, index) => {
      ...
      // Both taper to zero by the first neighbour, so only the centred card
      // stands taller and larger than the ring around it.
      const emphasis = Math.max(0, 1 - distance);
      const scale = 1 + activeScale * emphasis;
      const lift = activeLift * emphasis;

      card.style.transform =
        `translateX(calc(-50% + ${offset * pitch}px)) translateY(${-lift}px) ` +
  ...
  }, [activeLift, activeScale, count, depth, fade, falloff, gap, loop, rotate]);
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Pointer capture is deferred to the first real move — grabbing it here
    // would reroute the eventual click to the frame, so a card's onClick
    // would never fire for a plain tap.
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!drag.moved) {
      // A tap. Cards live in a `transform-style: preserve-3d` block inside
      // an `overflow: hidden` frame — a combination browsers frequently
      // fail to hit-test correctly, so `elementFromPoint` (and therefore
      // React's onClick) can land on the flattened ring instead of the
      // rotated card underneath. Resolve which card by position instead.
      const pitch = widthRef.current * (1 + gap);
      const frame = frameRef.current;
      if (pitch && frame) {
        const frameRect = frame.getBoundingClientRect();
        const localX = event.clientX - (frameRect.left + frameRect.width / 2);
        const rawIndex = Math.round(posRef.current + localX / pitch);
        goTo(((rawIndex % count) + count) % count);
      }
      return;
    }
  ...
  return (
    <div
      ...
      style={{
        ...
        ["--cf-card-h" as string]: cardHeight ?? cardWidth,
      }}
      role="region"
    ...
    >
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute top-0 left-1/2 overflow-hidden rounded-2xl shadow-xl will-change-transform",
      ...
      {showCaption && active?.title && (
        ...
        >
          <p className="text-[15px] tracking-tight text-foreground">
            {active.title}
          </p>
          {active.subtitle && (
